...
#define VERSION "2.42"
new g_MyMsgSync2, g_MyMsgSync3, g_MyMsgSync4, g_MyMsgSync1
...
public plugin_init()
{
	...
	kz_uq_connect = register_cvar("kz_uq_connect", "abdeghijklmno");
	...
	g_MyMsgSync1 = CreateHudSyncObj()
	g_MyMsgSync2 = CreateHudSyncObj()
	g_MyMsgSync3 = CreateHudSyncObj()
	g_MyMsgSync4 = CreateHudSyncObj()
}
...
stock kz_make_cvarexec(const config[])
{
	...
	fprintf(f, "kz_uq_connects ^"%s^"^n", stringscvars);
	fprintf(f, "^n");
					...
					for( new i = INFO_ONE; i < max_players; i++ )
					{
						...
						{	
							...
							if(jump_type[id] != Type_Slide && streifstat[id]==true && jump_type[id]!=Type_None && !failed_jump[id])
							{
								...
								ShowSyncHudMsg(i, g_MyMsgSync1,"%L",LANG_SERVER,"UQSTATS_HUD_PRINT7",strMessBuf); //stata streifof
							}
							...
							
							{
								...
								ShowSyncHudMsg(i, g_MyMsgSync2, "%L",LANG_SERVER,"UQSTATS_HUD_PRINT7",strMBuf);//stata duckov
							}
							...
							if(jump_type[id] != Type_Slide && streifstat[id]==true && jump_type[id]!=Type_None && (failed_jump[id]))
							{
								...
								ShowSyncHudMsg(i, g_MyMsgSync3, "%L",LANG_SERVER,"UQSTATS_HUD_PRINT7",strMessBuf);  //stata streifof fail
							}
							
							if(kz_stats_pre[id]==true && (jump_type[id]==Type_Multi_CountJump || (multiscj[id]==2 && jump_type[id]==Type_StandUp_CountJump) || (multidropcj[id]==2 && jump_type[id] == Type_Drop_CountJump)) && (failed_jump[id]))
							{
								set_hudmessage(f_stats_r, f_stats_g, f_stats_b, duck_x,duck_y, 0, 6.0, 2.5, 0.1, 0.3, h_duck);	
								ShowSyncHudMsg(i, g_MyMsgSync4, "%L",LANG_SERVER,"UQSTATS_HUD_PRINT7",strMBuf); //stata duckov fail
							}					
						}
					}
							...
							if( !failed_jump[id] )
							{
										     ...
										     || jump_type[id]==Type_ladderBhop || jump_type[id]==Type_ladder || jump_type[id]==Type_LongJump || jump_type[id]==Type_HighJump))
								{
									if ( distance[id] >= god_dist ) {
										...
										{
											client_cmd(ids, "speak misc/4");
										}
										if( uq_light ) krasnota(id);
										CC_SendMessage(ids, "!y[!gKZ!y] !g%s : %s !y| Distance: !g%2.1f !y| Pre: !g%2.1f !y| Strafe: !g%d !y| Sync: !g%d%% | %s", g_playername[id], Jtype1[id], distance[id], prestrafe[id], strafe_num[id], sync_[id], pre_type[id])
									}
									else if ( distance[id] >= leet_dist  ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/2");
									
										if( uq_light ) krasnota(id);
										CC_SendMessage(ids, "!y[!gKZ!y] !g%s : %s !y| Distance: !g%2.1f !y| Pre: !g%2.1f !y| Strafe: !g%d !y| Sync: !g%d%% | %s", g_playername[id], Jtype1[id], distance[id], prestrafe[id], strafe_num[id], sync_[id], pre_type[id])
									}
									else if ( distance[id] >= holy_dist ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/3");
									
										if( uq_light ) sineva(id);
										CC_SendMessage(ids, "!y[!gKZ!y] !g%s : %s !y| Distance: !g%2.1f !y| Pre: !g%2.1f !y| Strafe: !g%d !y| Sync: !g%d%% | %s", g_playername[id], Jtype1[id], distance[id], prestrafe[id], strafe_num[id], sync_[id], pre_type[id])
									}
									else if ( distance[id] >= pro_dist ) {
										if( uq_sounds && enable_sound[id]==true ) client_cmd(id, "speak misc/1");
								...
								else if(jump_type[id]==Type_Multi_CountJump || (multiscj[id]==2 && jump_type[id]==Type_StandUp_CountJump) || (multidropcj[id]==2 && jump_type[id]==Type_Drop_CountJump))
								{
									if ( distance[id] >= god_dist ) {
										if( uq_sounds && enable_sound[ids]==true )
										{
											client_cmd(ids, "speak misc/4.wav");
										}
										if( uq_light ) krasnota(id);
...
public client_connect( id )
{
	...
	kz_uq_connect = register_cvar("kz_uq_connect", "abdeghijklmno");
	static connectt[33];
	get_pcvar_string(kz_uq_connect, connectt, 33);
...
stock CC_SendMessage(const id, const input[], any:...)
{
